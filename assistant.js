...
function formatAIResponse(data) {
    ...
    if ((task_type === 'image' || task_type === 'unknown') && content && content.primary_url) {
        return `
            ✅ Image generated by <strong>${executed_by.toUpperCase()}</strong><br><br>
            ...
            <details style="margin-top: 10px; font-size: 0.85em; opacity: 0.8;">
                ...
                <p><strong>Claude voted:</strong> ${arbitration.claude_vote.recommended}<br>
                <em>${arbitration.claude_vote.reasoning || ''}</em></p>
                <p><strong>Grok voted:</strong> ${arbitration.grok_vote.recommended}<br>
                <em>${arbitration.grok_vote.reasoning || ''}</em></p>
                <p><strong>Decision:</strong> ${arbitration.decision}</p>
    ...
    if (typeof content === 'string') {
        return `
            ${content.replace(/\n/g, '<br>')}<br><br>
            <details style="margin-top: 10px; font-size: 0.85em; opacity: 0.8;">
                <summary>🤖 Handled by ${executed_by.toUpperCase()}</summary>
                <p><strong>Claude voted:</strong> ${arbitration.claude_vote.recommended}</p>
                <p><strong>Grok voted:</strong> ${arbitration.grok_vote.recommended}</p>
                <p><strong>Cost:</strong> $${costs.total.toFixed(4)}</p>
            </details>
        `;
    }
    ...
    // Fallback
    return `Response from ${executed_by}: ${JSON.stringify(content)}`;
}
